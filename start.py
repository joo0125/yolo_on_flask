...
def detect(source):
	...
	model = torch.load('weights/best.pt', map_location=device)['model']
		...
		for i, det in enumerate(pred):  # detections per image
			...
			if det is not None and len(det):
				...
				for c in det[:, -1].unique():
					...
					listDet = ['Addict eat the peach', 'Kiehl-s Musk Eau de Toilette Spray', 'SantaMariaNovella-Angeldifirenze']

					if(str(names[int(c)]) in listDet):
						print("this is :" +  str(names[int(c)]))
